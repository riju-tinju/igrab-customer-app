            ...
            >
                ...
                <div className={`flex items-center justify-between px-6 py-6 border-b border-gray-100 ${isRTL ? 'flex-row-reverse' : ''}`}>
                    <div className={isRTL ? 'text-right' : 'text-left'}>
                        <h2 className="text-2xl font-bold text-text-black font-chillax">{t('wishlist_title')}</h2>
                        {wishlistItems.length > 0 && (
                            <p className="text-xs text-gray-400 font-inter mt-1">
                                {wishlistItems.length} {t(wishlistItems.length === 1 ? 'item_saved' : 'items_saved')}
                            </p>
                        )}
                    </div>
                    <button
                    ...
                    {wishlistItems.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-center">
                            <div className="w-24 h-24 rounded-full bg-gray-50 flex items-center justify-center mb-6">
                                <i className="fas fa-heart text-gray-200 text-4xl"></i>
                            </div>
                            <h3 className="text-xl font-bold text-gray-900 mb-2">{t('wishlist_empty')}</h3>
                            <p className="text-sm text-gray-400 font-inter mb-8 max-w-[240px]">{t('wishlist_empty_desc')}</p>
                            <Link
                                to="/shop"
                                onClick={onClose}
                        ...
                        wishlistItems.map(item => (
                            <div key={item.id} className={`flex items-center gap-4 py-2 group ${isRTL ? 'flex-row-reverse' : ''}`}>
                                ...
                                <div className={`flex-1 min-w-0 ${isRTL ? 'text-right' : 'text-left'}`}>
                                    ...
                                    <p className="font-bold text-primary text-base" dir="ltr">{item.currency} {item.price.toFixed(2)}</p>
                                </div>
                                <div className={`flex flex-col items-end gap-3 ${isRTL ? 'items-start' : 'items-end'}`}>
                                    ...
                                    <button
                                        ...
                                        className={`text-xs px-4 py-2 rounded-xl font-bold transition-all shadow-sm active:scale-95 ${isInCart(item.id)
                                            ? 'bg-green-50 text-green-600 border border-green-200 cursor-default'
                                            : 'bg-primary text-white hover:bg-primary/90'
                                            }`}
                                    >
                                        {isInCart(item.id) ? t('in_cart_tick') : t('add_to_cart')}
                                    </button>
                                </div>
                            </div>
                        ))
                ...
                {wishlistItems.length > 0 && (
                    <div className="border-t border-gray-100 px-6 py-6 bg-gray-50/30">
                        ...
                        >
                            {t('move_all_to_cart')}
                            <i className="fas fa-shopping-cart text-sm opacity-70"></i>
                        </button>
                    </div>
                )}
            </div>
